...
export const Home = () => {
        ...
        <div className="relative max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
          <div>
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              <button onClick={() => setOpenPhoneModal(true)} className="px-6 py-3 bg-brand-primary text-brand-text-light font-semibold rounded-full shadow-lg hover:bg-brand-primary/90 transition-transform transform hover:scale-105">
                Reservar Cita
              </button>
          ...
          {openPhoneModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              <div className="absolute inset-0 backdrop-blur-xs bg-black/40" onClick={() => setOpenPhoneModal(false)} aria-hidden></div>
              <div role="dialog" aria-modal="true" aria-labelledby="modal-title" className="relative bg-white rounded-2xl shadow-xl max-w-sm w-full p-6">
                <h3 id="modal-title" className="text-lg font-semibold text-brand-highlight">Reservar Cita</h3>
                ...
                <div className="mt-4 flex flex-col gap-4 py-4 items-center justify-between p-3 border-t border-b border-gray-100">
                  ...
                  <div className="flex items-center gap-2">
                    <a href={`tel:${phoneNumber.replace(/\s+/g, '')}`} className="inline-flex text-brand-highlight items-center px-4 py-2 bg-brand-primary rounded-full shadow-lg hover:bg-brand-primary/90 transition">
                      Llamar
                    </a>
                    <button onClick={copyNumber} className="inline-flex items-center px-4 py-2 bg-brand-secondary text-brand-highlight rounded-full shadow-lg border border-brand-secondary hover:bg-brand-secondary/80 transition">
                      Copiar
                    </button>
                  </div>
                </div>
  ...
  <p className="mt-4 text-brand-highlight max-w-xl mx-auto">Tu momento de relax y belleza te está esperando. No dudes en contactarnos para cualquier consulta o para reservar tu cita.</p>
        <button onClick={() => setOpenPhoneModal(true)} className="text-brand-highlight mt-8 px-8 py-3 bg-brand-primary font-semibold rounded-full shadow-lg hover:bg-brand-primary/90 transition-transform transform hover:scale-105 duration-300 ease-in-out">
